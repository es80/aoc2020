...
applyInstruction program state =
  let
    newPrevIndices     = IntSet.insert currIndex $ prevIndices state
    currAcc            = acc state
    currIndex          = nextIndex state
    (currentOp, delta) = program !! currIndex
  in
    case currentOp of
      "acc" -> state { acc         = currAcc + delta
                     , nextIndex   = currIndex + 1
                     , prevIndices = newPrevIndices
                     }
      "jmp" ->
        state { nextIndex = currIndex + delta, prevIndices = newPrevIndices }
      "nop" ->
        state { nextIndex = currIndex + 1, prevIndices = newPrevIndices }
