...
orientateCorner (sym : syms) tiles =
  case findMatch matchLeftToRight sym tiles of
...
updateImage tiles image =
  let zipped = zip [0 ..] tiles
      newAssocs =
        map (\(i, tile) -> tileForImage tile (i `div` 12) (i `mod` 12)) zipped
  in  image // concat newAssocs

removeBorders :: Array Index Char -> Array Index Char
removeBorders image =
  let
    associations = assocs image
...
getImage tiles =
  let assembled = uncurry fill (getCorner tiles)
  in  removeBorders $ updateImage assembled mkImage
...
checkAllForSeaMonster image seaMonster =
  let ind = indices image
  in  mapMaybe (\i -> lookForSeaMonster i image seaMonster) ind

getOrientation :: Array Index Char -> SeaMonster -> Array Index Char
getOrientation image seaMonster =
  let orientations = getSymmetries image in pick orientations
 where
  pick [] = error "failed"
  pick (x : xs) =
    if null $ checkAllForSeaMonster x seaMonster then pick xs else x
