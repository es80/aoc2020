...
formSingleMap xs =
  ...
  in  if length xs == Map.size singleMap then singleMap else error "Parsed duplicate bags."
...
getCount coloursMap ((colour, multiple) : restOfBags) = case Map.lookup colour coloursMap of
  Nothing       -> multiple + getCount coloursMap restOfBags
  Just moreBags -> multiple * numNewBags + getCount coloursMap
                                                    (restOfBags ++ map multiplier moreBags)
   where
    numNewBags = sum (map snd moreBags)
    multiplier = (\(colour, currentMultiple) -> (colour, currentMultiple * multiple))
