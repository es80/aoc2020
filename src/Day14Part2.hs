...
bitsToNum []       = 0
bitsToNum (b : bs) = let n = bitsToNum bs in if b == 1 then n + 2 ^ length bs else n
...
processPartProgram memory (mask, (address, value) : rest) =
  let bits       = pad $ numToBits address
      masked     = applyMask mask bits
      newValues  = map bitsToNum masked
      mapUpdates = map (, value) newValues
      newMap     = Map.fromList mapUpdates
  in  processPartProgram (Map.union newMap memory) (mask, rest)
