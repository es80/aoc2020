      ...
      in  case maybeHead ply1 of
            ...
            Just c1 -> case maybeHead ply2 of
              ...
              Just c2 -> if length rest1 >= c1 && length rest2 >= c2
                then
                  let (subWinner, _) =
                        playRounds (take c1 rest1, take c2 rest2) (Set.empty, Set.empty)
                  in  case subWinner of
                        PlayerOne -> playRounds (rest1 ++ [c1, c2], rest2) newPrevious
                        PlayerTwo -> playRounds (rest1, rest2 ++ [c2, c1]) newPrevious
                else if c1 > c2
                  then playRounds (rest1 ++ [c1, c2], rest2) newPrevious
                  else playRounds (rest1, rest2 ++ [c2, c1]) newPrevious

getWinningDeck :: (Deck1, Deck2) -> [Int]
getWinningDeck startingDecks = snd $ playRounds startingDecks (Set.empty, Set.empty)
