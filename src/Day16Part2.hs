...
filterValid rules ticket = if all (checkValidAnyRule rules) ticket then Just ticket else Nothing
...
combineAll tickets rules =
  let allSets = map (ticketPossibleFields rules) tickets in foldl1 combineTickets allSets
...
checkAllSingle []       = ()
checkAllSingle (x : xs) = if Set.size x /= 1 then error "Elimination failed" else checkAllSingle xs
