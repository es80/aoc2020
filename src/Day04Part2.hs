...
splitAtFirst p [] = ([], [])
splitAtFirst p (x : xs) =
  let (first, second) = splitAtFirst p xs in if p x then ([], xs) else (x : first, second)
...
validKeys = Set.fromList ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"]
...
validYear (low, high) string = intervalContainsInt (low, high) (read string :: Int)
...
validHeight string =
  ...
  in  numMatches == 2 && validHeightHelper (read $ head matches :: Int, matches !! 1)
