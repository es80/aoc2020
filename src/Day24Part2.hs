...
adjacentLocations location = Set.fromList
  $ map (moveToAdjacentLocation location) [East, West, NorthEast, NorthWest, SouthEast, SouthWest]
...
tilesToFlipWhite tiles =
  Set.filter (\l -> let num = getNumAdjacentBlacks l tiles in num == 0 || num > 2) tiles

tilesToFlipBlack :: BlackTiles -> Set Location
tilesToFlipBlack tiles =
  Set.filter (\l -> getNumAdjacentBlacks l tiles == 2) (getWhiteTileLocations tiles)
