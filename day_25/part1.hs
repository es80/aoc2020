...
transformN steps subNum val =
  transformN (steps - 1) subNum (transform subNum val)
...
getLoopSizes val loopSize (cardPubKey, doorPubKey)
  ...
  | otherwise = getLoopSizes (transform val subjectNum)
                             (loopSize + 1)
                             (cardPubKey, doorPubKey)

getEncryptionKey :: (Int, Int) -> Int
getEncryptionKey (cardPubKey, doorPubKey) =
  let (cardLoopSize, doorLoopSize) =
        getLoopSizes subjectNum startVal (cardPubKey, doorPubKey)
